...
export default function HeroBanner({ homedata }: HeroBannerProps) {
  const mediaUrl = getMediaUrl(homedata.image?.url);
  const mediaIsVideo = isVideo(homedata.image?.mime, homedata.image?.url);
    ...
    <section className="relative h-screen w-full overflow-hidden flex items-end" aria-label="Hero">
      ...
      <div className="absolute inset-0 z-0">
        {mediaIsVideo ? (
          ...
          <Image
            src={mediaUrl}
            alt={homedata.image?.alternativeText ?? "Hero background"}
            fill
            priority
